...
object PlumeStatistics extends Enumeration {
  ...
  val TIME_OPEN_DRIVER, TIME_CLOSE_DRIVER, TIME_EXTRACTION, TIME_REACHABLE_BY_QUERYING, TIME_REMOVING_OUTDATED_GRAPH,
    TIME_REMOVING_OUTDATED_CACHE, TIME_RETRIEVING_CACHE, TIME_STORING_CACHE, PROGRAM_CLASSES, PROGRAM_METHODS = Value
  ...
  /** Measures the time for a given code block and saves it under the [[PlumeStatistic]] heading.
    * @param statistic
    *   the statistic to save the elapsed time under.
    * @param block
  ...
  /** Sets the value of a statistic.
    ...
    * @param newValue
    *   The new value to associate with the statistic.
    */
  ...
  /** The results of the measured time per process.
    * @return
    *   a map of each [[PlumeStatistic]] and the time per process in nanoseconds.
    */
